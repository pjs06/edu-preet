...
const PHOTOSYNTHESIS_CONTENT = {
    ...
    STORY: {
        ...
        videoUrl: 'https://www.youtube.com/embed/K8jA6h57Lz8',
        duration: '6:00',
        questions: [
            {
                id: 'sq1',
    ...
    ANALOGY: {
        ...
        videoUrl: 'https://www.youtube.com/embed/3pD68uxRLkM',
        duration: '7:00',
        questions: [
            {
                id: 'aq1',
...
async function seedAdaptiveContent() {
    const client = await pool.connect();
    try {
        console.log('Seeding adaptive content for Photosynthesis...');

        // We'll use a fixed concept_id for "Photosynthesis" (module 1)
        // In a real app, we'd fetch this from the curriculum table
        const conceptId = 'module_001';
        const language = 'en';

        await client.query('BEGIN');

        // Insert each path
        const paths = ['MAIN', 'STORY', 'ANALOGY'];

        for (const type of paths) {
            const content = PHOTOSYNTHESIS_CONTENT[type];
            const pathType = type.toLowerCase(); // main, story, analogy

            // Generate a dummy hash
            const promptHash = crypto.createHash('md5').update(JSON.stringify(content)).digest('hex');
            ...
            await client.query(`
                ...
                VALUES ($1, $2, $3, $4, $5, $6)
                ON CONFLICT (concept_id, path_type, language, prompt_hash) 
                DO UPDATE SET 
            ...
            `, [
                ...
                language,
                promptHash,
                JSON.stringify(content), // Storing the full content object
                JSON.stringify({
                    title: content.title,
            ...
            console.log(`✅ Seeded ${type} path`);
        }

        await client.query('COMMIT');
        console.log('🎉 Adaptive content seeding complete!');
